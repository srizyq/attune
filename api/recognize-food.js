...
Macros are grams, calories are kcal, all rounded to whole numbers except macros can have one decimal. fibre and sugar are grams, sodium is milligrams.`;
}
...
export default async function handler(req, res) {
  ...
  try {
    const response = await client.messages.create({
      ...
      // (content = [thinking], no text block, surfaced as "Couldn't read a
      // response for this photo"). Confirmed against the real API on
      // recognize-menu.js's identical pattern. This is a single-shot
      // structured-JSON extraction with no need for exposed reasoning, so
      // disabling thinking removes the failure mode entirely.

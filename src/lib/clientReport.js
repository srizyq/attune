...
// The food_logs column behind each micronutrient key. Every MICRO_NUTRIENTS
// entry must appear here — clientReport.test.js fails if one is added without.
export const MICRO_COLUMNS = {
  fibre: 'fibre_g', sodium: 'sodium_mg', sugar: 'sugar_g', saturatedFat: 'saturated_fat_g',
  transFat: 'trans_fat_g', cholesterol: 'cholesterol_mg', addedSugar: 'added_sugar_g', potassium: 'potassium_mg',
  vitaminD: 'vitamin_d_mcg', calcium: 'calcium_mg', iron: 'iron_mg', vitaminA: 'vitamin_a_mcg',
  vitaminC: 'vitamin_c_mg', vitaminB12: 'vitamin_b12_mcg', folate: 'folate_mcg', magnesium: 'magnesium_mg',
  zinc: 'zinc_mg', polyunsaturatedFat: 'polyunsaturated_fat_g', monounsaturatedFat: 'monounsaturated_fat_g',
};
...
export function buildReport({ client, start, end, dayFilter = 'all', foodLogs = [], checkins = [], weightLogs = [], workouts = [], includeMicros = false }) {
  ...
  const micros = includeMicros && included.length > 0
    ? MICRO_NUTRIENTS.map((n) => ({
        ...
        avg: round(mean(included.map((d) => d.micros[n.key] || 0)), n.unit === 'g' || n.unit === 'mg' ? 1 : 0),
      }))
    : null;
...
export function buildCsv(report) {
  ...
  const rows = report.included.map((d) => [
    ...
    d.mood, d.energy, d.water, ...micros.map((n) => round(d.micros[n.key] || 0, 1)),
  ]);
...
${micros ? `<h2>Average daily micronutrients</h2>${table(['Nutrient', 'Average'], micros.map((n) => [n.label, `${n.avg} ${n.unit}`]))}` : ''}
...
<p class="note">Figures come from what the client logged in Attune. Foods without micronutrient data in their source count as zero, so micronutrient averages can understate intake; a "complete day" is inferred as one with 3 or more of the 4 meals logged. Generated by Attune.</p>
</body></html>`;
}
